...
Node * delete_node(int num,Node *head)

{

Node *current_node=head;

while ( current_node != NULL && current_node->next->data !=num)
...
if(current_node->next->next!=NULL) current_node->next=current_node->next->next;

else current_node->next=NULL; return head;
...
Node *current_node = head; 
 while ( current_node != NULL) { printf("%d ", current_node->data); current_node = current_node->next;
...
printf("\n Choose an option [1-7] : "); if (scanf("%d", &option) != 1) { printf(" *Error: Invalid input. Try again.\n"); scanf("%s", &temp); /*clear input buffer */
...
case 3: /* Insert After */ printf(" Enter a number to insert : "); if (scanf("%d", &num) != 1) { printf(" *Error: Invalid input.\n");
...
case 4: /* Insert Before */ printf(" Enter a number to insert : "); if (scanf("%d", &num) != 1) { printf(" *Error: Invalid input. \n");
...
case 6: /* delete a node */ printf("enter the no which have to delete:");

scanf("%d",&num); head=delete_node(num,head); printf("The node is deleted");

break; case 7: /* Exit */ return(0); break;
